...
void EndVirtualScissorMode(void) { EndScissorMode(); }

static Rectangle VisibleVirtualScreenRect(void) {
  float scale = (g_scale > 0.0f) ? g_scale : 1.0f;
  return (Rectangle){-g_mouseOffset.x / scale, -g_mouseOffset.y / scale,
...
static void DrawVisibleVirtualGradientV(Color top, Color bottom) {
  ...
  DrawRectangleGradientV((int)floorf(r.x), (int)floorf(r.y),
                         (int)ceilf(r.width), (int)ceilf(r.height), top,
                         bottom);
}
...
bool AnySaveExistsCached(void) {
  ...
  if (now - lastCheck > 1.0) {
    ...
    for (int i = 1; i <= SAVE_SLOT_COUNT; i++) {
      char path[64];
      if (i == AUTO_SAVE_SLOT)
        snprintf(path, sizeof(path), "Saves/auto_save.txt");
      else
        snprintf(path, sizeof(path), "Saves/save_slot_%d.txt", i);
      if (FileExists(path)) {
        cached = true;
        break;
      }
...
// bounds são a ÚNICA fonte de verdade (desenho e hitbox usam o mesmo
// retângulo). "JOGAR" abre a seleção de dificuldade; o seletor antigo foi
// removido.
...
// ============================================================================
void DrawTextFitCentered(Font font, const char *text, Rectangle area,
                         float maxFont, Color color, bool shadow) {
  float fs = maxFont;
  float padding = 16.0f;
  ...
  Vector2 pos = {area.x + (area.width - sz.x) / 2.0f,
                 area.y + (area.height - sz.y) / 2.0f};
  if (shadow)
    DrawTextEx(font, text, (Vector2){pos.x + 1.5f, pos.y + 1.5f}, fs, 1.0f,
               Fade(BLACK, 0.55f));
...
void DrawButton(UIButton botao, Font font, bool enabled) {
  extern Vector2 g_virtualMouse;
  bool pressed = enabled && botao.hover &&
  ...
  Rectangle r = {cx - b.width * sc / 2.0f, cy - b.height * sc / 2.0f,
                 b.width * sc, b.height * sc};
  ...
  if (!enabled) {
    ...
    DrawRectangleRounded(
        (Rectangle){r.x - 5, r.y - 5, r.width + 10, r.height + 10}, 0.3f, 8,
        Fade(THEME_COLOR_MAIN, 0.12f * pulse + 0.05f));
  ...
  if (enabled && botao.hover && !pressed) {
    ...
    DrawRectangle((int)r.x + 3, (int)(r.y + r.height * 0.2f), 3,
                  (int)(r.height * 0.6f), Fade(THEME_COLOR_MAIN, pulse));
  }
...
static void ScreenPalette(int screen, Color *top, Color *bot, Color *accent) {
  switch (screen) {
  case SCREEN_ARSENAL:
    *top = (Color){14, 16, 22, 255};
    *bot = (Color){8, 12, 16, 255};
    *accent = (Color){255, 200, 80, 255};
    break;
  case SCREEN_SKINS:
    *top = (Color){20, 10, 28, 255};
    *bot = (Color){10, 6, 18, 255};
    *accent = (Color){200, 110, 255, 255};
    break;
  case SCREEN_TUTORIAL:
  case SCREEN_CONTROLS:
    *top = (Color){8, 18, 28, 255};
    *bot = (Color){5, 10, 18, 255};
    *accent = (Color){0, 200, 255, 255};
    break;
  case SCREEN_SETTINGS:
    *top = (Color){10, 18, 20, 255};
    *bot = (Color){6, 10, 12, 255};
    *accent = (Color){0, 229, 200, 255};
    break;
  case SCREEN_ADMIN:
    *top = (Color){22, 8, 8, 255};
    *bot = (Color){10, 4, 5, 255};
    *accent = (Color){255, 80, 80, 255};
    break;
  case SCREEN_GAMEOVER:
    *top = (Color){22, 6, 12, 255};
    *bot = (Color){8, 3, 6, 255};
    *accent = (Color){230, 60, 70, 255};
    break;
  case SCREEN_VICTORY:
    *top = (Color){10, 16, 36, 255};
    *bot = (Color){6, 8, 20, 255};
    *accent = (Color){255, 210, 90, 255};
    break;
  default:
    *top = THEME_COLOR_BG_DARK;
    *bot = THEME_COLOR_BG_LIGHT;
    *accent = THEME_COLOR_MAIN;
    break;
  }
}
...
void DrawThemedBackground(int screen, float time, float entry) {
  ...
  static Color cTop = {6, 18, 10, 255}, cBot = {10, 28, 18, 255},
               cAcc = {0, 229, 255, 255};
  static int inited = 0;
  Color tTop, tBot, tAcc;
  ScreenPalette(screen, &tTop, &tBot, &tAcc);
  if (!inited) {
    cTop = tTop;
    cBot = tBot;
    cAcc = tAcc;
  ...
  float k = 6.0f * GetFrameTime();
  if (k > 1.0f)
    k = 1.0f;
  cTop = ColLerp(cTop, tTop, k);
  cBot = ColLerp(cBot, tBot, k);
  cAcc = ColLerp(cAcc, tAcc, k);
  ...
  for (int i = 0; i < 26; i++) {
    ...
    float y = baseY - fmodf(time * (12.0f + (i % 5) * 6.0f),
  ...
  DrawRectangleGradientV(0, 0, SCREEN_WIDTH, 90, Fade(BLACK, 0.35f),
                         Fade(BLACK, 0.0f));
  DrawRectangleGradientV(0, SCREEN_HEIGHT - 90, SCREEN_WIDTH, 90,
                         Fade(BLACK, 0.0f), Fade(BLACK, 0.35f));
}
...
void DrawPanel(Rectangle r, Color accent, float bgAlpha) {
  ...
  DrawLineEx((Vector2){r.x + r.width, r.y}, (Vector2){r.x + r.width - len, r.y},
             th, accent);
  DrawLineEx((Vector2){r.x + r.width, r.y}, (Vector2){r.x + r.width, r.y + len},
             th, accent);
  DrawLineEx((Vector2){r.x, r.y + r.height},
             (Vector2){r.x + len, r.y + r.height}, th, accent);
  DrawLineEx((Vector2){r.x, r.y + r.height},
             (Vector2){r.x, r.y + r.height - len}, th, accent);
  DrawLineEx((Vector2){r.x + r.width, r.y + r.height},
             (Vector2){r.x + r.width - len, r.y + r.height}, th, accent);
  DrawLineEx((Vector2){r.x + r.width, r.y + r.height},
             (Vector2){r.x + r.width, r.y + r.height - len}, th, accent);
}

void DrawTitleText(Font font, const char *text, float centerX, float y,
  ...
  for (int gx = -2; gx <= 2; gx++)
    for (int gy = -2; gy <= 2; gy++)
      if (gx || gy)
        DrawTextEx(font, text, (Vector2){pos.x + gx * 2.0f, pos.y + gy * 2.0f},
                   fontSize, 2.0f, Fade(color, 0.05f * glow));
  DrawTextEx(font, text, (Vector2){pos.x + 3, pos.y + 3}, fontSize, 2.0f,
             Fade(BLACK, 0.5f));
  DrawTextEx(font, text, pos, fontSize, 2.0f, color);
}

float DrawTextWrapped(Font font, const char *text, Rectangle area,
                      float fontSize, float spacing, Color color) {
  int n = 0;
  char buf[1024];
  for (const char *p = text; *p && n < 1023; p++)
    buf[n++] = *p;
  buf[n] = '\0';
  ...
  for (float fs = fontSize; fs >= 9.0f; fs -= 1.0f) {
    ...
    for (char *q = copy;; q++) {
      if (*q == ' ' || *q == '\0') {
        ...
        *q = '\0';
        ...
        if (x > area.x && x + ws.x > area.x + area.width) {
          x = area.x;
          y += lineH;
        }
        x += ws.x + spaceW;
    ...
    if (y + lineH > area.y + area.height && fs > 9.0f)
      continue; // não coube: encolhe
    ...
    for (char *q = draw;; q++) {
      if (*q == ' ' || *q == '\0') {
        ...
        *q = '\0';
        ...
        if (dx > area.x && dx + ws.x > area.x + area.width) {
          dx = area.x;
          dy += lineH;
        }
...
void DrawTooltip(Font font, const char *text, Vector2 anchor) {
  ...
  Vector2 sz = MeasureTextEx(font, text, fs, 1.0f);
  Rectangle r = {anchor.x + 14.0f, anchor.y + 14.0f, sz.x + pad * 2.0f,
                 sz.y + pad * 2.0f};
  if (r.x + r.width > SCREEN_WIDTH - 6.0f)
    r.x = anchor.x - r.width - 14.0f;
  if (r.y + r.height > SCREEN_HEIGHT - 6.0f)
    r.y = SCREEN_HEIGHT - r.height - 6.0f;
...
// ============================================================================
void DrawUICard(Rectangle r, Color accent, bool hover, bool selected,
                float entry) {
  ...
  if (selected) {
    ...
    DrawRectangleRoundedLines(
        (Rectangle){r.x - 3, r.y - 3, r.width + 6, r.height + 6}, 0.07f, 10,
        Fade(accent, (0.35f + 0.45f * gp) * entry));
    DrawRectangleRoundedLines(r, 0.07f, 10, Fade(accent, entry));
  } else {
    DrawRectangleRoundedLines(r, 0.07f, 10,
                              Fade(accent, (hover ? 0.9f : 0.5f) * entry));
  }
  // Faixa de acento no topo.
  DrawRectangleRounded((Rectangle){r.x + 12, r.y + 10, r.width - 24, 5}, 0.8f,
                       4, Fade(accent, entry));
}

void DrawUITab(Font font, Rectangle r, const char *text, Color accent,
               bool active, bool hover) {
  Color bg =
      active ? Fade(accent, 0.22f) : Fade((Color){12, 14, 22, 255}, 0.85f);
  Color br =
      active ? accent : (hover ? Fade(accent, 0.8f) : THEME_COLOR_BORDER);
  DrawRectangleRounded(r, 0.3f, 6, bg);
  DrawRectangleRoundedLines(r, 0.3f, 6, br);
  DrawTextFitCentered(font, text, r, 18.0f,
                      active ? accent : (hover ? WHITE : Fade(WHITE, 0.8f)),
                      true);
  if (active)
    DrawRectangleRounded((Rectangle){r.x + r.width * 0.2f, r.y + r.height - 4,
                                     r.width * 0.6f, 3},
                         0.8f, 4, accent);
}
...
void DrawUIScreenTitle(Font font, const char *text, Color accent, float entry) {
  ...
  float lw = sz.x * 0.6f;
  DrawRectangleRounded(
      (Rectangle){SCREEN_WIDTH / 2.0f - lw / 2.0f, y + 50.0f, lw, 3.0f}, 0.8f,
      4, Fade(accent, 0.7f * entry));
}
...
void DrawUIStatBar(Font font, float x, float y, float w, const char *label,
                   float v, const char *valueTxt, Color col) {
  v = Clamp(v, 0.0f, 1.0f);
  ...
  DrawRectangleRoundedLines((Rectangle){x, by, w, bh}, 0.7f, 6,
                            Fade(col, 0.7f));
  if (valueTxt) {
...
void DrawUIInput(Font font, Rectangle r, const char *text,
                 const char *placeholder, bool active, bool hover,
                 bool showCursor) {
  ...
  while (ts.x > r.width - 24.0f && fs > 10.0f) {
    fs -= 1.0f;
    ts = MeasureTextEx(font, shown, fs, 1.0f);
  }
  Vector2 pos = {r.x + r.width / 2.0f - ts.x / 2.0f,
                 r.y + r.height / 2.0f - ts.y / 2.0f};
  DrawTextEx(font, shown, pos, fs, 1.0f, empty ? Fade(GRAY, 0.55f) : WHITE);
  if (active && showCursor)
    DrawRectangle((int)(pos.x + ts.x + 3), (int)pos.y, 2, (int)ts.y,
                  THEME_COLOR_MAIN);
}

void DrawUISectionPanel(Font font, Rectangle r, const char *title, Color accent,
                        float entry) {
  DrawRectangleRounded(r, 0.05f, 8,
                       Fade((Color){8, 11, 18, 255}, 0.75f * entry));
  DrawRectangleRoundedLines(r, 0.05f, 8, Fade(accent, 0.5f * entry));
  if (title) {
    DrawTextEx(font, title, (Vector2){r.x + 16, r.y + 10}, 22.0f, 1.0f,
               Fade(accent, entry));
    DrawLineEx((Vector2){r.x + 16, r.y + 38},
               (Vector2){r.x + r.width - 16, r.y + 38}, 1.0f,
               Fade(accent, 0.4f * entry));
  }
}
...
void DrawUIToast(Font font, const char *text, Color accent, float alpha) {
  ...
  Vector2 sz = MeasureTextEx(font, text, fs, 1.0f);
  Rectangle r = {SCREEN_WIDTH / 2.0f - sz.x / 2.0f - pad, 120.0f,
                 sz.x + pad * 2.0f, sz.y + pad};
  DrawRectangleRounded(r, 0.4f, 8,
                       Fade((Color){8, 12, 18, 255}, 0.92f * alpha));
  DrawRectangleRoundedLines(r, 0.4f, 8, Fade(accent, alpha));
  DrawTextEx(font, text, (Vector2){r.x + pad, r.y + pad / 2.0f}, fs, 1.0f,
             Fade(WHITE, alpha));
}

Rectangle SettingsMusicVolumeTrack(void) {
  return (Rectangle){555.0f, 292.0f, 300.0f, 16.0f};
}
Rectangle SettingsSfxVolumeTrack(void) {
  return (Rectangle){555.0f, 348.0f, 300.0f, 16.0f};
}
...
void DrawMenuVirus(Vector2 c, float radius, float rotationDeg, Color col) {
  ...
  for (int i = 0; i < spikes; i++) {
    ...
    Vector2 base = {c.x + cosf(a) * radius * 0.72f,
                    c.y + sinf(a) * radius * 0.72f};
    Vector2 tip = {c.x + cosf(a) * radius * 1.18f,
                   c.y + sinf(a) * radius * 1.18f};
...
void DrawMenuBacteria(Vector2 c, float size, float angleDeg, Color col) {
  ...
  for (int i = -2; i <= 2; i++) {
    Vector2 p = {c.x + dir.x * (size * 0.45f * i),
                 c.y + dir.y * (size * 0.45f * i)};
    DrawLineEx(
        p, (Vector2){p.x + perp.x * size * 0.55f, p.y + perp.y * size * 0.55f},
        1.5f, Fade(col, 0.4f));
    DrawLineEx(
        p, (Vector2){p.x - perp.x * size * 0.55f, p.y - perp.y * size * 0.55f},
        1.5f, Fade(col, 0.4f));
...
void DrawMenuBiohazard(Vector2 c, float radius, float pulse, Color col) {
  ...
  for (int k = 0; k < 3; k++) // 3 lâminas (crescentes)
  {
    ...
    DrawRing(c, r * 0.46f, r, base + 18.0f, base + 102.0f, 24,
             Fade(col, 0.85f));
  }
...
void DrawMenuBackground(Color accent, float time, float entry) {
  ...
  // Micróbios decorativos nas BORDAS (posições determinísticas; deixam o centro
  // livre). {x, y, tamanho, tipo(0=virus,1=bact), corIndex}
  static const float decor[][5] = {
      {70, 90, 30, 0, 0},    {150, 280, 22, 1, 1},  {60, 470, 26, 0, 2},
      {120, 640, 20, 1, 1},  {1210, 80, 32, 0, 0},  {1130, 250, 24, 1, 1},
      {1225, 430, 22, 0, 2}, {1150, 610, 28, 1, 1}, {300, 60, 18, 1, 1},
      {980, 60, 20, 0, 0},   {360, 670, 18, 0, 2},  {940, 668, 20, 1, 1},
  };
  ...
  for (int i = 0; i < n; i++) {
    Vector2 c = {decor[i][0],
                 decor[i][1] + sinf(time * 0.6f + i) *
                                   8.0f}; // flutuação lenta + parallax
    float sz = decor[i][2];
    Color col = (decor[i][4] == 0)   ? virusCol
                : (decor[i][4] == 1) ? bactCol
                                     : cyanCol;
    col = Fade(col, fade);
    if ((int)decor[i][3] == 0)
      DrawMenuVirus(c, sz, time * 25.0f + i * 30.0f, col); // vírus girando
    else
      DrawMenuBacteria(c, sz, time * 12.0f + i * 40.0f,
                       col); // bactéria flutuando
  }

  // Símbolo de risco biológico pulsante (canto inferior direito, vermelho
  // coral).
  DrawMenuBiohazard((Vector2){1180, 560}, 46.0f,
                    0.5f + 0.5f * sinf(time * 2.0f),
                    Fade((Color){255, 90, 90, 255}, 0.5f * fade));
}

void DrawNeonTitle(Font font, float centerX, float topY, float scale,
                   float entry, float time) {
  ...
  for (int li = 0; li < 2; li++) {
    ...
    for (int rad = 12; rad >= 4; rad -= 4)
      for (int d = 0; d < 8; d++) {
        float a = d * (PI / 4.0f);
        DrawTextEx(font, lines[li],
                   (Vector2){x + cosf(a) * rad, y + sinf(a) * rad}, fs, 4.0f,
                   Fade(neon, 0.05f * e));
      }
    // Sombra escura + contorno.
    DrawTextEx(font, lines[li], (Vector2){x + 4, y + 5}, fs, 4.0f,
               Fade(BLACK, 0.55f * e));
    for (int dx = -2; dx <= 2; dx += 2)
      for (int dy = -2; dy <= 2; dy += 2)
        if (dx || dy)
          DrawTextEx(font, lines[li], (Vector2){x + dx, y + dy}, fs, 4.0f,
                     Fade(deep, 0.6f * e));
    // Preenchimento neon + brilho interno.
    DrawTextEx(font, lines[li], (Vector2){x, y}, fs, 4.0f, Fade(neon, e));
    DrawTextEx(font, lines[li], (Vector2){x, y - 1}, fs, 4.0f,
               Fade((Color){200, 255, 210, 255}, 0.35f * e));
  }
}

bool DrawMenuBanner(Rectangle area, float entry) {
  if (!SpriteAvailable(SPR_UI_MENU_BANNER))
    return false;
  ...
  float scale =
      fminf(area.width / (float)t.width, area.height / (float)t.height);
...
static unsigned MfxRandU(void) {
  MFX.rng = MFX.rng * 1664525u + 1013904223u;
  return MFX.rng;
}
static float MfxRandF(void) { return (MfxRandU() >> 8) / 16777216.0f; }
static float MfxRange(float a, float b) { return a + (b - a) * MfxRandF(); }

// Desenha uma textura centralizada (organismos do catálogo / biohazard /
// seringa).
static void DrawTexCentered(Texture2D t, Vector2 c, Vector2 sz, float rot,
                            Color tint) {
  if (t.id == 0)
    return;
  Rectangle src = {0, 0, (float)t.width, (float)t.height};
...
static Color MenuOrgColor(int catIdx) {
  if (catIdx < 0 || catIdx >= MenuOrganismCount())
    return (Color){120, 220, 160, 255};
  const char *c = MENU_ORGANISMS[catIdx].color;
  if (c[0] == 'g')
    return (Color){90, 230, 120, 255}; // green
  if (c[0] == 'b')
    return (Color){70, 200, 255, 255}; // blue
  return (Color){190, 110, 255, 255};  // purple
}

// Escolhe o modelo de organismo MENOS representado (mantém os 18 sempre
// visíveis).
...
static int MenuAllowedCatAt(int allowedIndex) {
  return (allowedIndex >= MENU_ORGANISM_EXCLUDED) ? allowedIndex + 1
                                                  : allowedIndex;
}

// Posiciona/reseta um organismo por toda a tela, longe das seringas e dos
// demais organismos. O painel e o título são desenhados depois, portanto
// continuam legíveis mesmo quando um elemento atravessa a região central ao
// fundo.
static void MenuOrgPlace(MenuOrganism *o, int forceCat, bool fadeIn) {
  ...
  for (int tries = 0; tries < 24; tries++) {
    ...
    for (int i = 0; ok && i < MFX.orgCount; i++) {
      ...
      if (!other->active || other == o || other->isBiohazard)
        continue;
      float dx = zx - other->position.x, dy = zy - other->position.y;
      if (dx * dx + dy * dy < 72.0f * 72.0f)
        ok = false;
    }
    if (ok)
      break;
  ...
  o->velocity =
      (Vector2){cosf(moveAngle) * moveSpeed, sinf(moveAngle) * moveSpeed};
  o->tint = WHITE;
...
static void MenuSyringeSpawn(MenuSyringe *s) {
  float speed = MfxRange(
      48.0f, 72.0f); // travessia calma, ainda mais rápida que os organismos
  int edge = (int)(MfxRandF() * 4.0f);
  Vector2 p, v;
  if (edge == 0) {
    p = (Vector2){-100.0f, MfxRange(80, 640)};
    v = (Vector2){speed, MfxRange(-40, 40)};
  } else if (edge == 1) {
    p = (Vector2){1380.0f, MfxRange(80, 640)};
    v = (Vector2){-speed, MfxRange(-40, 40)};
  } else if (edge == 2) {
    p = (Vector2){MfxRange(140, 1140), -100.0f};
    v = (Vector2){MfxRange(-50, 50), speed};
  } else {
    p = (Vector2){MfxRange(140, 1140), 820.0f};
    v = (Vector2){MfxRange(-50, 50), -speed};
  }
  s->position = p;
  s->velocity = v;
...
static void MenuSpawnExplosion(Vector2 at, Color base) {
  ...
  for (int k = 0; k < count; k++) {
    ...
    p->color =
        (Color){(unsigned char)fminf(255, base.r + MfxRange(0, 40)),
...
static void MenuFX_Init(void) {
  ...
  if (n <= 0)
    n = 18;
  int allowedCount =
      n - ((MENU_ORGANISM_EXCLUDED >= 0 && MENU_ORGANISM_EXCLUDED < n) ? 1 : 0);
  for (int i = 0; i < target; i++) {
    MenuOrganism o = (MenuOrganism){0};
    // Os primeiros slots garantem que todos os modelos permitidos apareçam;
    // o restante repete modelos variados (escala/profundidade/rotação
    // distintas).
  ...
  for (int i = 0; i < MFX.syrCount; i++)
    MenuSyringeSpawn(&MFX.syr[i]);

  for (int i = 0; i < MENU_PART_MAX; i++)
    MFX.part[i].active = false;
  MFX.init = true;
}
...
static void MenuFX_Update(float dt, float time) {
  if (dt > 0.1f)
    dt = 0.1f; // estabilidade se houver hitch
  ...
  for (int i = 0; i < MFX.bioStart + MFX.bioCount; i++) {
    ...
    if (!o->isBiohazard && !o->active) {
      if (o->respawnTimer > 0.0f)
        o->respawnTimer -= dt;
      if (o->respawnTimer <= 0.0f)
        MenuOrgPlace(o, -1, true); // reaparece com fade
      continue;
    }
    if (o->fade < 1.0f) {
      o->fade += dt * 2.2f;
    ...
    if (o->basePosition.y < -margin)
      o->basePosition.y = SCREEN_HEIGHT + margin;
    if (o->basePosition.y > SCREEN_HEIGHT + margin)
      o->basePosition.y = -margin;
    if (o->basePosition.x < -margin)
      o->basePosition.x = SCREEN_WIDTH + margin;
    if (o->basePosition.x > SCREEN_WIDTH + margin)
      o->basePosition.x = -margin;
    o->position.x =
        o->basePosition.x + sinf(time * 0.6f + o->phase) * 9.0f * o->depth;
    o->position.y = o->basePosition.y +
                    cosf(time * 0.5f + o->phase * 1.3f) * 7.0f * o->depth;
  ...
  for (int s = 0; s < MFX.syrCount; s++) {
    ...
    Vector2 perp = {-dir.y, dir.x};
    Vector2 vel = Vector2Add(
        sy->velocity, Vector2Scale(perp, sinf(sy->wobble * 1.2f) * 7.0f));
    sy->position = Vector2Add(sy->position, Vector2Scale(vel, dt));
    sy->rotation = atan2f(vel.y, vel.x) * RAD2DEG + 90.0f;
    if (sy->position.x < -170 || sy->position.x > 1450 ||
        sy->position.y < -170 || sy->position.y > 890)
      MenuSyringeSpawn(sy);
  }
  ...
  for (int s = 0; s < MFX.syrCount; s++) {
    MenuSyringe *sy = &MFX.syr[s];
    if (!sy->active || sy->cooldown > 0.0f)
      continue;
    Texture2D st = GetSprite(SPR_MENU_SYRINGE);
    float syrH = (70.0f + sy->depth * 60.0f) * sy->scale;
    float syrR = (st.id ? syrH : 60.0f) * 0.26f;
    for (int i = 0; i < MFX.orgCount;
         i++) // só organismos (biohazard não é destruído)
    {
      MenuOrganism *o = &MFX.org[i];
      if (!o->active)
        continue;
      float dx = o->position.x - sy->position.x,
            dy = o->position.y - sy->position.y;
  ...
  while (activeCount < MENU_ORGANISM_MIN_ACTIVE) {
    ...
    float bestT = 1e9f;
    for (int i = 0; i < MFX.orgCount; i++)
      if (!MFX.org[i].active && MFX.org[i].respawnTimer < bestT) {
        bestT = MFX.org[i].respawnTimer;
        best = i;
      }
    if (best < 0)
      break;
...
// Desenha organismos cujo `depth` está em [dmin, dmax) (camada de
// profundidade).
static void MenuFX_DrawOrganisms(float dmin, float dmax, float time,
                                 float entry) {
  for (int i = 0; i < MFX.bioStart + MFX.bioCount; i++) {
    MenuOrganism *o = &MFX.org[i];
    if (!o->isBiohazard && !o->active)
      continue;
    if (o->depth < dmin || o->depth >= dmax)
      continue;

    float glow = 0.85f + 0.15f * sinf(time * 2.0f + o->phase);
    float pulse = 1.0f + 0.03f * sinf(time * 1.8f + o->phase); // pulsação sutil
    Texture2D t = o->isBiohazard ? GetSprite(SPR_MENU_BIOHAZARD)
                                 : GetMenuOrganism(o->catIdx);
    if (t.id != 0 && t.height > 0) {
      float targetH = (o->isBiohazard ? 64.0f : 48.0f) + o->depth * 40.0f;
      float sc =
          targetH / t.height * pulse; // escala uniforme (preserva proporção)
      Vector2 sz = {t.width * sc, t.height * sc};
      float a = (0.42f + 0.55f * o->depth) * entry * glow * o->fade;
      if (o->isBiohazard)
        a *= 0.85f;
      DrawTexCentered(t, o->position, sz, o->rotation,
                      Fade(WHITE, a)); // tint WHITE: cor original
    } else if (o->isBiohazard) {
      DrawMenuBiohazard(o->position, 30.0f * o->depth + 18.0f, glow,
                        Fade((Color){255, 70, 70, 255}, entry));
    }
  }
}
...
static void MenuFX_DrawSyringes(float dmin, float dmax, float entry) {
  ...
  for (int s = 0; s < MFX.syrCount; s++) {
    MenuSyringe *sy = &MFX.syr[s];
    if (!sy->active || sy->depth < dmin || sy->depth >= dmax)
      continue;
...
static void MenuFX_DrawParticles(void) {
  for (int i = 0; i < MENU_PART_MAX; i++) {
    ...
    DrawCircleV(p->position, r * 0.5f,
                Fade((Color){235, 255, 245, 255}, a * 0.7f));
  }
}

// Título DISEASE'S / DOOMSDAY montado letra a letra a partir dos glifos da
// arte.
static void MenuFX_DrawTitle(Font font, float centerX, float topY,
                             float screenAnim, float time) {
  if (!MenuTitleGlyphsReady()) {
    DrawNeonTitle(font, centerX, topY, 1.0f, screenAnim / 0.5f,
                  time); // fallback (fonte)
    return;
  }
  float scale = 0.92f;
  ...
  float collective =
      0.02f * (0.5f + 0.5f * sinf(time * 1.2f)); // pulso coletivo sutil (tempo)
  float sweep =
      fmodf(time * 0.45f, 1.6f); // brilho atravessando as letras (0..1.6)
  for (int i = 0; i < MENU_TITLE_GLYPH_COUNT; i++) {
    ...
    float squash =
        1.0f + sinf(time * 4.0f + ph) * 0.03f + collective; // compressão/pulso
    ...
    // Brilho atravessando (light sweep): realce branco quando o sweep passa
    // pelo glifo.
    float gp = (float)i / (float)MENU_TITLE_GLYPH_COUNT;
    float d = fabsf(sweep - gp);
    if (d < 0.10f)
      DrawTexturePro(
          t, src, dst, (Vector2){0, 0}, 0.0f,
          Fade((Color){220, 255, 230, 255}, (0.10f - d) / 0.10f * 0.5f * ge));
  }
}
...
void DrawTelaMenu(GameState *game, Font font, float time) {
  ...
  case SCREEN_ARSENAL:
    morphAccent = (Color){0, 220, 255, 255};
    break; // ciano
  case SCREEN_SKINS:
    morphAccent = (Color){190, 110, 255, 255};
    break; // roxo
  case SCREEN_CONTROLS:
    morphAccent = (Color){90, 220, 120, 255};
    break; // verde
  case SCREEN_SETTINGS:
    morphAccent = (Color){80, 150, 255, 255};
    break; // azul
  case SCREEN_ADMIN:
    morphAccent = (Color){255, 120, 110, 255};
    break; // vermelho suave
  default:
    morphAccent = THEME_COLOR_MAIN;
    break;
  }
  ...
  // Fundo azul-marinho quase preto + brilho de acento no topo + vinheta
  // inferior.
  Rectangle visible = VisibleVirtualScreenRect();
  DrawVisibleVirtualGradientV((Color){9, 13, 30, 255}, (Color){4, 6, 16, 255});
  DrawRectangleGradientV(
      (int)floorf(visible.x), (int)floorf(visible.y), (int)ceilf(visible.width),
      150, Fade(curAccent, (0.05f + 0.03f * sinf(time * 1.5f)) * entry),
      Fade(curAccent, 0.0f));
  DrawRectangleGradientV(
      (int)floorf(visible.x), SCREEN_HEIGHT - 90, (int)ceilf(visible.width),
      (int)ceilf(visible.y + visible.height - (SCREEN_HEIGHT - 90)),
  ...
  for (int i = 0; i < MAX_PARTICLES; i++) {
    if (game->particles[i].active) {
      ...
      DrawRectangleV(
          (Vector2){game->particles[i].position.x - s,
                    game->particles[i].position.y - s},
          (Vector2){s * 2.0f, s * 2.0f},
          Fade(game->particles[i].color,
               game->particles[i].lifeTime / game->particles[i].maxLifeTime));
    }
  }
  ...
  DrawTextEx(font,
             "Seu Anticorpo:", (Vector2){nameBounds.x + 5, nameBounds.y - 18},
             14.0f, 1.0f, GRAY);
  ...
  if (game->player.name[0] == '\0') {
    if (game->nameInputActive) {
      ...
      Vector2 textSz =
          MeasureTextEx(font, "NOME DO ANTICORPO...", (float)fontSize, 1.0f);
      Vector2 textPos = {
          nameBounds.x + (nameBounds.width / 2.0f) - (textSz.x / 2.0f),
          nameBounds.y + (nameBounds.height / 2.0f) - (textSz.y / 2.0f)};
      DrawTextEx(font, "NOME DO ANTICORPO...", textPos, (float)fontSize, 1.0f,
                 Fade(GRAY, 0.5f));
    }
  } else {
  ...
  // Se existe arquivo de save em qualquer um dos 3 slots, indica que pode
  // carregar
  ...
  DrawTextEx(font, hint, (Vector2){640.0f - hs.x / 2.0f, SCREEN_HEIGHT - 24.0f},
             13.0f, 1.0f, Fade(THEME_COLOR_TEXT, 0.55f));
}
...
static Rectangle DiffStartRect(void) {
  return (Rectangle){SCREEN_WIDTH / 2.0f - 175.0f, 572.0f, 350.0f, 52.0f};
}
static Rectangle DiffBackRect(void) {
  return (Rectangle){40.0f, 652.0f, 220.0f, 46.0f};
}
...
static void DiffIntBar(Font font, float x, float y, float w, const char *label,
                       float v, Color col, float a) {
  v = Clamp(v, 0.0f, 1.0f);
  DrawTextEx(font, label, (Vector2){x, y}, 13.0f, 1.0f, Fade(WHITE, 0.8f * a));
  float by = y + 17.0f, bh = 9.0f;
  DrawRectangleRounded((Rectangle){x, by, w, bh}, 0.7f, 4,
                       Fade(BLACK, 0.5f * a));
...
void DrawTelaDifficulty(GameState *game, Font font) {
  ...
  int pd = game->pendingDifficulty;
  if (pd < 0 || pd > 2)
    pd = DIFFICULTY_MEDIUM;
  DrawMenuBackground(DIFF_COL[pd], time, game->screenAnim / 0.5f);

  DrawTitleText(font, "SELECIONE A DIFICULDADE", SCREEN_WIDTH / 2.0f, 70.0f,
                40.0f, Fade(THEME_COLOR_TEXT, entry));
  const char *sub =
      "A dificuldade so e aplicada ao confirmar em INICIAR MISSAO.";
  Vector2 ss = MeasureTextEx(font, sub, 16.0f, 1.0f);
  DrawTextEx(font, sub, (Vector2){SCREEN_WIDTH / 2.0f - ss.x / 2.0f, 124.0f},
             16.0f, 1.0f, Fade(WHITE, 0.8f * entry));

  const char *names[3] = {"FACIL", "MEDIO", "DIFICIL"};
  const char *subt[3] = {"Acessivel", "Recomendado", "Extremo"};
  const char *desc[3] = {
      "Inimigos um pouco mais lentos e janelas de esquiva generosas. Ideal "
      "para aprender e curtir a campanha.",
      "Desafio elevado: IA mais agressiva e coordenada, com invocacoes mais "
      "frequentes. A experiencia recomendada.",
      "Experiencia extrema para veteranos: reacao quase instantanea, esquiva e "
      "flanqueamento implacaveis."};
  const char *barLabels[3] = {"IA / coordenacao", "Velocidade",
                              "Pressao / cadencia"};
  float barVals[3][3] = {
      {0.45f, 0.55f, 0.40f}, {0.72f, 0.74f, 0.68f}, {1.00f, 0.95f, 1.00f}};
  ...
  for (int i = 0; i < 3; i++) {
    ...
    float dy = (1.0f - ce) * 50.0f + ((hover || selected) ? -6.0f : 0.0f) +
               (pressed ? 3.0f : 0.0f);
    ...
    DrawRectangleRounded(
        dr, 0.06f, 10,
        Fade((Color){10, 13, 20, 255}, (selected ? 0.95f : 0.82f) * ce));
    if (selected) {
      float gp = 0.5f + 0.5f * sinf(time * 5.0f);
      DrawRectangleRoundedLines(
          (Rectangle){dr.x - 3, dr.y - 3, dr.width + 6, dr.height + 6}, 0.06f,
          10, Fade(col, (0.4f + 0.5f * gp) * ce));
      DrawRectangleRoundedLines(dr, 0.06f, 10, Fade(col, ce));
    } else {
      DrawRectangleRoundedLines(dr, 0.06f, 10,
                                Fade(col, (hover ? 0.9f : 0.5f) * ce));
    }
    // Faixa de acento no topo do card.
    DrawRectangleRounded((Rectangle){dr.x + 14, dr.y + 12, dr.width - 28, 6},
                         0.8f, 4, Fade(col, ce));

    float pad = 22.0f;
    // Nome (ajustado para nunca estourar).
    DrawTextFitCentered(font, names[i],
                        (Rectangle){dr.x, dr.y + 26, dr.width, 40}, 32.0f,
                        Fade(col, ce), true);
    // Badge RECOMENDADO no Médio.
    if (i == 1) {
      const char *bd = "RECOMENDADO";
      Vector2 bs = MeasureTextEx(font, bd, 12.0f, 1.0f);
      Rectangle badge = {dr.x + dr.width / 2.0f - bs.x / 2.0f - 10, dr.y + 64,
                         bs.x + 20, 20};
      DrawRectangleRounded(badge, 0.5f, 6, Fade(col, 0.22f * ce));
      DrawRectangleRoundedLines(badge, 0.5f, 6, Fade(col, ce));
      DrawTextEx(font, bd, (Vector2){badge.x + 10, badge.y + 4}, 12.0f, 1.0f,
                 Fade(col, ce));
    }
    // Subtítulo.
    Vector2 sts = MeasureTextEx(font, subt[i], 17.0f, 1.0f);
    DrawTextEx(font, subt[i],
               (Vector2){dr.x + dr.width / 2.0f - sts.x / 2.0f, dr.y + 92},
               17.0f, 1.0f, Fade(WHITE, 0.85f * ce));
    // Descrição (wrap; encolhe p/ caber na altura).
    DrawTextWrapped(font, desc[i],
                    (Rectangle){dr.x + pad, dr.y + 120, dr.width - pad * 2, 92},
                    15.0f, 1.0f, Fade(WHITE, 0.8f * ce));
    // Indicador de intensidade (3 barras).
    DrawTextEx(font, "INTENSIDADE", (Vector2){dr.x + pad, dr.y + 222}, 13.0f,
               1.0f, Fade(col, ce));
    for (int b = 0; b < 3; b++)
      DiffIntBar(font, dr.x + pad, dr.y + 244 + b * 30, dr.width - pad * 2,
                 barLabels[b], barVals[i][b], col, ce);
    // Selecionado.
    if (selected) {
      const char *sel = "> SELECIONADO <";
      Vector2 zs = MeasureTextEx(font, sel, 14.0f, 1.0f);
      DrawTextEx(font, sel,
                 (Vector2){dr.x + dr.width / 2.0f - zs.x / 2.0f,
                           dr.y + dr.height - 28},
                 14.0f, 1.0f, Fade(col, ce));
    }
  }

  // Botões: INICIAR MISSAO (confirma) + VOLTAR.
  UIButton startBtn = {DiffStartRect(), "INICIAR MISSAO",
                       CheckCollisionPointRec(g_virtualMouse, DiffStartRect()),
                       false};
  DrawButton(startBtn, font, true);
  UIButton backBtn = {DiffBackRect(), "VOLTAR",
                      CheckCollisionPointRec(g_virtualMouse, DiffBackRect()),
                      false};
  DrawButton(backBtn, font, true);

  const char *nav =
      "Setas / A-D para escolher   -   ENTER confirma   -   ESC volta";
  Vector2 ns = MeasureTextEx(font, nav, 14.0f, 1.0f);
  DrawTextEx(
      font, nav,
      (Vector2){SCREEN_WIDTH / 2.0f - ns.x / 2.0f, SCREEN_HEIGHT - 26.0f},
      14.0f, 1.0f, Fade(THEME_COLOR_TEXT, 0.6f));
}
...
void UpdateTelaDifficulty(GameState *game, Vector2 mouse) {
  ...
  if (backClick || IsKeyPressed(KEY_ESCAPE)) {
    ...
    if (ret == SCREEN_DIFFICULTY_SELECT)
      ret = SCREEN_MENU; // segurança
...
static void DrawTutorialIcon(int tab, Vector2 c, float size, Color accent,
                             float time) {
  ...
  case 0: // controles: direcional
    DrawRectangleRounded(
        (Rectangle){c.x - s * 0.12f, c.y - s * 0.38f, s * 0.24f, s * 0.76f},
        0.3f, 4, accent);
    DrawRectangleRounded(
        (Rectangle){c.x - s * 0.38f, c.y - s * 0.12f, s * 0.76f, s * 0.24f},
        0.3f, 4, accent);
    break;
  case 1: // arma/projétil
    DrawLineEx((Vector2){c.x - s * 0.32f, c.y + s * 0.24f},
               (Vector2){c.x + s * 0.28f, c.y - s * 0.30f}, s * 0.16f, accent);
  ...
  case 2: // patógeno
    ...
    for (int i = 0; i < 8; i++) {
      ...
      DrawLineEx(a0, a1, 2.0f, accent);
      DrawCircleV(a1, s * 0.05f, accent);
    }
    break;
  case 3: // chefe/coroa
    DrawTriangle((Vector2){c.x - s * .34f, c.y + s * .20f},
                 (Vector2){c.x - s * .24f, c.y - s * .30f},
                 (Vector2){c.x, c.y + s * .02f}, accent);
    DrawTriangle((Vector2){c.x, c.y + s * .02f},
                 (Vector2){c.x + s * .24f, c.y - s * .30f},
                 (Vector2){c.x + s * .34f, c.y + s * .20f}, accent);
    DrawRectangleRounded(
        (Rectangle){c.x - s * .34f, c.y + s * .15f, s * .68f, s * .17f}, 0.3f,
        4, accent);
    break;
  case 4: // skin/escudo
    ...
    break;
  case 5: // evolução
    DrawLineEx((Vector2){c.x - s * .28f, c.y + s * .28f},
               (Vector2){c.x + s * .28f, c.y - s * .28f}, 4.0f, accent);
    DrawTriangle((Vector2){c.x + s * .28f, c.y - s * .28f},
                 (Vector2){c.x + s * .02f, c.y - s * .22f},
                 (Vector2){c.x + s * .22f, c.y + s * .02f}, accent);
    DrawCircleV((Vector2){c.x - s * .25f, c.y + s * .25f}, s * .09f, WHITE);
    break;
  default: // hordas/ondas
    for (int i = 0; i < 3; i++)
      DrawRing(c, s * (0.12f + i * .10f), s * (0.15f + i * .10f), 200, 340, 18,
               Fade(accent, 1.0f - i * .18f));
    break;
  }
}

// Linha "rótulo: texto" auxiliar do tutorial
static void TutLine(Font font, float x, float y, const char *label, Color lc,
                    const char *text) {
  DrawTextEx(font, label, (Vector2){x, y}, 18.0f, 1.0f, lc);
  if (text && text[0]) {
    float lw = MeasureTextEx(font, label, 18.0f, 1.0f).x;
    DrawTextEx(font, text, (Vector2){x + lw + 10.0f, y}, 18.0f, 1.0f,
               Fade(WHITE, 0.85f));
  }
}
...
static void DrawTutorialEnemyPreview(int type, Vector2 center, float size,
                                     bool boss) {
  Enemy preview = {0};
...
static void DrawEnemyGuideRow(Font font, Rectangle row, int type,
  ...
  if (type == ETYPE_BACT_RANGED)
    previewRadius = 13.5f;
  DrawTutorialEnemyPreview(type,
                           (Vector2){row.x + 27.0f, row.y + row.height * 0.5f},
                           previewRadius, false);
  DrawTextEx(font, name, (Vector2){row.x + 54.0f, row.y + 7.0f}, 15.0f, 1.0f,
             color);
  Vector2 ws = MeasureTextEx(font, wave, 11.0f, 1.0f);
  DrawTextEx(font, wave,
             (Vector2){row.x + row.width - ws.x - 9.0f, row.y + 9.0f}, 11.0f,
             1.0f, Fade(GOLD, 0.9f));
  if (row.height >= 70.0f) {
    DrawTextWrapped(font, role,
                    (Rectangle){row.x + 54.0f, row.y + 29.0f, row.width - 64.0f,
                                row.height - 35.0f},
                    12.0f, 1.0f, Fade(WHITE, 0.78f));
  } else {
    DrawTextEx(font, role, (Vector2){row.x + 54.0f, row.y + 29.0f}, 12.0f, 1.0f,
               Fade(WHITE, 0.78f));
  }
}
...
static void DrawTutorialWeaponCard(Font font, Rectangle card, int weapon,
  ...
  Rectangle preview = {card.x + 14.0f, card.y + 18.0f, card.width * 0.43f,
                       card.height - 36.0f};
  float bob = sinf(time * 2.0f + weapon) * 6.0f;
  Vector2 pc = {preview.x + preview.width * 0.5f,
                preview.y + preview.height * 0.5f + bob};
  DrawCircleGradient(pc, preview.height * 0.54f, Fade(color, 0.22f), BLANK);
  DrawCircleLines((int)pc.x, (int)pc.y, preview.height * 0.44f,
                  Fade(color, 0.28f));
  DrawHeldWeaponFramed(
      weapon,
      (Rectangle){preview.x, preview.y + bob, preview.width, preview.height},
      58.0f, sinf(time * 1.7f + weapon) * 10.0f, primary, secondary);

  float tx = card.x + card.width * 0.49f;
  float tw = card.x + card.width - tx - 16.0f;
  DrawTextEx(font, slot, (Vector2){tx, card.y + 16.0f}, 14.0f, 1.0f,
             Fade(GOLD, 0.95f));
  DrawTextWrapped(font, wi.name, (Rectangle){tx, card.y + 38.0f, tw, 50.0f},
                  22.0f, 1.0f, color);
  DrawTextWrapped(font, wi.special, (Rectangle){tx, card.y + 92.0f, tw, 45.0f},
...
static void DrawTutContent(GameState *game, Font font, Rectangle panel) {
  ...
  case 0: // BASICO + CONTROLES
    DrawTextEx(font, "OBJETIVO", (Vector2){x, y}, 22.0f, 1.0f, accent);
    y += 30.0f;
    DrawTextEx(
        font,
        "Voce e um Anticorpo. Sobreviva a 5 hordas de patogenos, derrote o",
        (Vector2){x, y}, 17.0f, 1.0f, Fade(WHITE, 0.85f));
    y += 24.0f;
    DrawTextEx(font,
               "chefe da onda 5 e proteja o organismo do Distrito Federal.",
               (Vector2){x, y}, 17.0f, 1.0f, Fade(WHITE, 0.85f));
    y += 38.0f;
    DrawTextEx(font, "CONTROLES", (Vector2){x, y}, 22.0f, 1.0f, accent);
    y += 32.0f;
    TutLine(font, x, y, "WASD / SETAS", YELLOW, "Mover o Anticorpo");
    y += step;
    TutLine(font, x, y, "CLIQUE ESQ. / ESPACO", YELLOW,
            "Atacar com a arma equipada");
    y += step;
    TutLine(font, x, y, "1  2  3  4", YELLOW, "Selecionar / trocar de arma");
    y += step;
    TutLine(font, x, y, "E", YELLOW, "Usar pocao de vida (cura 50%)");
    y += step;
    TutLine(font, x, y, "ESC", YELLOW, "Pausar / voltar");
    y += step;
    TutLine(font, x, y, "F5 / F9", YELLOW, "Salvar / carregar rapido (slot 1)");
    y += step;
    TutLine(font, x, y, "POWER-UPS", GOLD,
            "Cura, Velocidade, Escudo e Dano x2 (brilham no mapa)");
    break;
  ...
  case 1: // ARMAS
  {
    ...
    if (g_tutWeaponsScroll < 0.0f)
      g_tutWeaponsScroll = 0.0f;
    if (g_tutWeaponsScroll > maxScroll)
      g_tutWeaponsScroll = maxScroll;

    float sy = y - g_tutWeaponsScroll;
    DrawTextEx(font,
               "Quatro slots. Cada arma evolui com 30 abates proprios. Role "
               "para ver tudo.",
               (Vector2){x, sy}, 18.0f, 1.0f, Fade(WHITE, 0.88f));
    sy += headerH;

    DrawTutorialWeaponCard(font, (Rectangle){x, sy, cardW, cardH}, 1, "SLOT 1",
                           "Combo alterna estocada frontal e corte rapido para "
                           "segurar inimigos perto.",
                           wp, ws, t);
    DrawTutorialWeaponCard(
        font, (Rectangle){x + cardW + gapX, sy, cardW, cardH}, WEAPON_BIOBLADE,
        "SLOT 1 / 30 ABATES",
        "Depois de desbloquear, aperte 1 para alternar com a Espada-Seringa.",
        wp, ws, t);
    sy += cardH + gapY;
    DrawTutorialWeaponCard(
        font, (Rectangle){x, sy, cardW, cardH}, 2, "SLOT 2 / NIVEL 2",
        "Tiro preciso para finalizar alvos em linha e controlar distancia.", wp,
        ws, t);
    DrawTutorialWeaponCard(
        font, (Rectangle){x + cardW + gapX, sy, cardW, cardH},
        WEAPON_RIFLE_EVOLVED, "SLOT 2 / 30 ABATES",
        "Projeteis replicantes criam um disparo extra no primeiro impacto.", wp,
        ws, t);
    sy += cardH + gapY;
    DrawTutorialWeaponCard(
        font, (Rectangle){x, sy, cardW, cardH}, 3, "SLOT 3 / NIVEL 3",
        "Planta minas no chao; aperte mouse 2 para detonar todas.", wp, ws, t);
    DrawTutorialWeaponCard(
        font, (Rectangle){x + cardW + gapX, sy, cardW, cardH},
        WEAPON_RNA_LAUNCHER, "SLOT 3 / 30 ABATES",
        "Arremessa minas a distancia; elas explodem em 6s ou com mouse 2.", wp,
        ws, t);
    sy += cardH + gapY;
    DrawTutorialWeaponCard(
        font, (Rectangle){x, sy, cardW, cardH}, 4, "SLOT 4 / NIVEL 4",
        "Projetil pesado perfurante para limpar corredores e chefes.", wp, ws,
        t);
    DrawTutorialWeaponCard(
        font, (Rectangle){x + cardW + gapX, sy, cardW, cardH},
        WEAPON_BFG_EVOLVED, "SLOT 4 / 30 ABATES",
    ...
    if (maxScroll > 0.0f) {
      ...
      float thumbY =
          trackY + (g_tutWeaponsScroll / maxScroll) * (trackH - thumbH);
      DrawRectangleRounded((Rectangle){trackX, trackY, 5.0f, trackH}, 0.8f, 4,
                           Fade(BLACK, 0.45f));
      DrawRectangleRounded((Rectangle){trackX, thumbY, 5.0f, thumbH}, 0.8f, 4,
                           Fade(accent, 0.78f));
    }
    break;
  }
  ...
  case 2: // INIMIGOS
  {
    const int virusTypes[5] = {ETYPE_VIRUS_SWARM, ETYPE_VIRUS_MELEE,
                               ETYPE_VIRUS_RANGED, ETYPE_VIRUS_ELITE,
                               ETYPE_VIRUS_BOSS};
    const char *virusWave[5] = {"ONDA 1", "ONDA 2", "ONDA 3", "ONDA 4",
                                "ONDA 5"};
    ...
    DrawTextEx(font, "MUNDO 2 - VIRUS", (Vector2){rightX, y}, 17.0f, 1.0f,
               accent);

    const int bacteriaTypes[3] = {ETYPE_BACT_MELEE, ETYPE_BACT_RANGED,
                                  ETYPE_KPC};
    const char *bacteriaWave[3] = {"ONDAS 1-5", "ONDAS 2-5", "ELITE / CHEFE"};
    const char *bacteriaRole[3] = {"Coco: persegue e ataca por contato.",
                                   "Bacilo: atirador que controla distancia.",
                                   "KPC: tanque resistente com tiro pesado."};
    for (int i = 0; i < 3; i++)
      DrawEnemyGuideRow(
          font, (Rectangle){leftX, y + 27.0f + i * 92.0f, 274.0f, 78.0f},
          bacteriaTypes[i], bacteriaWave[i], bacteriaRole[i]);

    for (int i = 0; i < 5; i++)
      DrawEnemyGuideRow(
          font, (Rectangle){rightX, y + 27.0f + i * 61.0f, 478.0f, 52.0f},
  ...
  case 3: // CHEFE
  {
    ...
    DrawRectangleRounded(bactCard, 0.08f, 8,
                         Fade((Color){35, 10, 14, 255}, 0.70f));
    DrawRectangleRoundedLines(bactCard, 0.08f, 8,
                              Fade((Color){255, 80, 90, 255}, 0.55f));
    DrawRectangleRounded(virusCard, 0.08f, 8,
                         Fade((Color){24, 8, 35, 255}, 0.70f));
    DrawRectangleRoundedLines(virusCard, 0.08f, 8,
                              Fade((Color){210, 90, 225, 255}, 0.60f));

    DrawTutorialEnemyPreview(
        ETYPE_KPC, (Vector2){bactCard.x + 49, bactCard.y + 54}, 27.0f, true);
    DrawTextEx(font, "MUNDO 1", (Vector2){bactCard.x + 92, bactCard.y + 18},
               13.0f, 1.0f, GOLD);
    DrawTextEx(font, "SUPERBACTERIA KPC",
               (Vector2){bactCard.x + 92, bactCard.y + 40}, 18.0f, 1.0f,
               (Color){255, 80, 90, 255});
    DrawTextEx(font, "Tanque resistente a antibioticos.",
               (Vector2){bactCard.x + 18, bactCard.y + 91}, 14.0f, 1.0f,
               Fade(WHITE, .82f));
    DrawTextEx(font, "Fase 1  Tiros pesados e escolta",
               (Vector2){bactCard.x + 18, bactCard.y + 124}, 14.0f, 1.0f,
               Fade(WHITE, .78f));
    DrawTextEx(font, "Fase 2  Acelera e invoca lacaios",
               (Vector2){bactCard.x + 18, bactCard.y + 151}, 14.0f, 1.0f,
               Fade(WHITE, .78f));
    DrawTextEx(font, "Fase 3  Rajada radial enfurecida",
               (Vector2){bactCard.x + 18, bactCard.y + 178}, 14.0f, 1.0f,
               Fade(WHITE, .78f));
    DrawTextEx(font, "Granada e BFG ajudam contra sua vida alta.",
               (Vector2){bactCard.x + 18, bactCard.y + 226}, 13.0f, 1.0f,
               (Color){120, 220, 140, 255});

    DrawTutorialEnemyPreview(ETYPE_VIRUS_BOSS,
                             (Vector2){virusCard.x + 51, virusCard.y + 54},
                             21.0f, true);
    DrawTextEx(font, "MUNDO 2", (Vector2){virusCard.x + 96, virusCard.y + 18},
               13.0f, 1.0f, GOLD);
    DrawTextEx(font, "CORONAVIRUS",
               (Vector2){virusCard.x + 96, virusCard.y + 40}, 18.0f, 1.0f,
               (Color){220, 90, 220, 255});
    DrawTextEx(font, "Chefe viral com capsideo reforcado.",
               (Vector2){virusCard.x + 18, virusCard.y + 91}, 14.0f, 1.0f,
               Fade(WHITE, .82f));
    DrawTextEx(font, "Fase 1  Quebre o escudo protetor",
               (Vector2){virusCard.x + 18, virusCard.y + 124}, 14.0f, 1.0f,
               Fade(WHITE, .78f));
    DrawTextEx(font, "Fase 2  Invoca enxames de rinovirus",
               (Vector2){virusCard.x + 18, virusCard.y + 151}, 14.0f, 1.0f,
               Fade(WHITE, .78f));
    DrawTextEx(font, "Fase 3  RNA exposto e rajada radial",
               (Vector2){virusCard.x + 18, virusCard.y + 178}, 14.0f, 1.0f,
               Fade(WHITE, .78f));
    DrawTextEx(font, "Lamina Bioeletrica rompe o capsideo.",
               (Vector2){virusCard.x + 18, virusCard.y + 226}, 13.0f, 1.0f,
               (Color){120, 220, 140, 255});

    DrawTextEx(font,
               "Ambos aparecem sempre na onda 5, protegidos por lacaios. "
               "Elimine os Nucleos de Infeccao.",
               (Vector2){x, y + 307.0f}, 14.0f, 1.0f, Fade(accent, 0.92f));
    break;
  }

  case 4: // SKINS
    DrawTextEx(font, "Abra 'SKINS' no menu para escolher (preview ao vivo).",
               (Vector2){x, y}, 17.0f, 1.0f, Fade(WHITE, 0.85f));
    y += 34.0f;
    DrawTextEx(font, "SKINS DO ANTICORPO:", (Vector2){x, y}, 18.0f, 1.0f,
               accent);
    y += 28.0f;
    TutLine(font, x + 16, y, "PADRAO", YELLOW, "cavaleiro imune classico.");
    y += 26.0f;
    TutLine(font, x + 16, y, "MEDICA", YELLOW,
            "jaleco branco com cruz vermelha.");
    y += 26.0f;
    TutLine(font, x + 16, y, "INFECTADA", YELLOW, "armadura roxa corrompida.");
    y += 34.0f;
    DrawTextEx(font,
               "SKIN DA ARMA muda a cor dos disparos (Padrao, Plasma, Toxica).",
               (Vector2){x, y}, 16.0f, 1.0f, Fade(WHITE, 0.8f));
    y += 26.0f;
    DrawTextEx(font, "As skins sao visuais e nao alteram o balanceamento.",
               (Vector2){x, y}, 15.0f, 1.0f, Fade(GRAY, 0.9f));
    break;

  case 5: // XP / UPGRADES
    DrawTextEx(font, "PROGRESSAO DE RPG", (Vector2){x, y}, 22.0f, 1.0f, accent);
    y += 32.0f;
    TutLine(font, x, y, "XP:", GOLD,
            "derrotar patogenos enche a barra roxa de XP.");
    y += 28.0f;
    TutLine(font, x, y, "NIVEL:", GOLD,
            "ao subir, ganha +Vida, +Dano, +Velocidade e cura total.");
    y += 28.0f;
    TutLine(font, x, y, "ARMAS:", GOLD,
            "Fuzil (Nv2), Granada (Nv3) e BFG (Nv4) desbloqueiam por nivel.");
    y += 28.0f;
    DrawTextEx(font, "PONTOS DO SUS", (Vector2){x, y}, 20.0f, 1.0f, accent);
    y += 28.0f;
    TutLine(font, x, y, "QUIZ:", GOLD,
            "acertar o quiz entre ondas da +50 Pontos do SUS.");
    y += 26.0f;
    TutLine(font, x, y, "LOJA:", GOLD,
            "gaste os pontos em +Vida Max, +Velocidade ou +Dano.");
    y += 26.0f;
    DrawTextEx(
        font,
        "A tela de Melhorias do SUS aparece automaticamente entre as ondas.",
        (Vector2){x, y}, 15.0f, 1.0f, Fade(WHITE, 0.8f));
    break;

  default: // HORDAS
    DrawTextEx(font, "SISTEMA DE HORDAS", (Vector2){x, y}, 22.0f, 1.0f, accent);
    y += 32.0f;
    DrawTextEx(
        font,
        "Sao 5 ondas, cada uma mais dificil: mais inimigos e mais fortes.",
        (Vector2){x, y}, 17.0f, 1.0f, Fade(WHITE, 0.85f));
    y += 26.0f;
    DrawTextEx(font, "Elimine todos os patogenos da onda para avancar.",
               (Vector2){x, y}, 17.0f, 1.0f, Fade(WHITE, 0.85f));
    y += 26.0f;
    DrawTextEx(font,
               "Entre as ondas: QUIZ educativo + tela de Melhorias do SUS.",
               (Vector2){x, y}, 17.0f, 1.0f, Fade(WHITE, 0.85f));
    y += 32.0f;
    TutLine(font, x, y, "ONDA 5:", (Color){255, 90, 90, 255},
            "batalha de CHEFE garantida (veja a aba CHEFE).");
    y += 30.0f;
    DrawTextEx(font,
               "Um banner no topo avisa cada nova horda e a chegada do chefe.",
               (Vector2){x, y}, 15.0f, 1.0f, (Color){120, 220, 140, 255});
    break;
  }
}
...
void DrawTelaControles(GameState *game, Font font) {
  ...
  for (int i = 0; i < TUT_TAB_COUNT; i++) {
    ...
    DrawTextFitCentered(font, g_tutTabNames[i], tabName, 16.0f,
                        active ? tabAccent : Fade(WHITE, hover ? 0.95f : 0.76f),
                        true);
    // Marcador de seleção (faixa vertical à esquerda, dentro da zona).
    if (active)
      DrawRectangleRounded(
          (Rectangle){dr.x + 4.0f, dr.y + 20.0f, 4.0f, dr.height - 28.0f}, 0.8f,
          4, tabAccent);
  }
  ...
  DrawTextEx(font, g_tutTabNames[g_tutTab], (Vector2){438.0f, 137.0f}, 26.0f,
             1.0f, Fade(acc, entry));
  DrawTextEx(font, g_tutTabSub[g_tutTab], (Vector2){438.0f, 170.0f}, 15.0f,
             1.0f, Fade(WHITE, 0.68f * entry));
  ...
  Rectangle progressBadge = {detail.x + detail.width - 104.0f, 137.0f, 76.0f,
                             30.0f};
  DrawRectangleRounded(progressBadge, 0.5f, 6,
                       Fade((Color){8, 14, 20, 255}, 0.78f * entry));
  ...
  DrawLineEx((Vector2){detail.x + 24.0f, 194.0f},
             (Vector2){detail.x + detail.width - 24.0f, 194.0f}, 2.0f,
             Fade(acc, 0.35f * entry));

  // Conteúdo com slide/fade ao trocar de categoria.
  static int prevTab = -1;
  static float tabAnim = 0.0f;
  if (g_tutTab != prevTab) {
    prevTab = g_tutTab;
    tabAnim = 0.0f;
  }
  ...
  const char *tutorialHint =
      "SETAS / W-S: navegar   |   clique para selecionar   |   ESC: voltar";
  DrawTextEx(font, tutorialHint, (Vector2){54.0f, 637.0f}, 15.0f, 1.0f,
             Fade(WHITE, 0.6f));
...
void UpdateTelaTutorial(GameState *game, Vector2 mouse) {
  int oldTab = g_tutTab;
  for (int i = 0; i < TUT_TAB_COUNT; i++) {
    if (CheckCollisionPointRec(mouse, TutTabRect(i)) &&
        IsMouseButtonPressed(MOUSE_LEFT_BUTTON))
      g_tutTab = i;
  }
  if (IsKeyPressed(KEY_RIGHT) || IsKeyPressed(KEY_DOWN) ||
      IsKeyPressed(KEY_D) || IsKeyPressed(KEY_S))
    g_tutTab = (g_tutTab + 1) % TUT_TAB_COUNT;
  if (IsKeyPressed(KEY_LEFT) || IsKeyPressed(KEY_UP) || IsKeyPressed(KEY_A) ||
      IsKeyPressed(KEY_W))
    g_tutTab = (g_tutTab + TUT_TAB_COUNT - 1) % TUT_TAB_COUNT;
  if (g_tutTab != oldTab && g_tutTab == 1)
    g_tutWeaponsScroll = 0.0f;
...
void DrawTelaPausa(GameState *game, Font font) {
  ...
  DrawRectangleRounded((Rectangle){420, 130, 440, 350}, 0.05f, 6,
                       Fade((Color){12, 8, 22, 255}, 0.92f));
  DrawRectangleRoundedLines((Rectangle){420, 130, 440, 350}, 0.05f, 6,
                            THEME_COLOR_BORDER);

  const char *pauseTxt = "GAME PAUSED";
  Vector2 pauseSize = MeasureTextEx(font, pauseTxt, 28.0f, 1.0f);
  DrawTextEx(font, pauseTxt,
             (Vector2){(SCREEN_WIDTH / 2.0f) - (pauseSize.x / 2.0f), 155.0f},
             28.0f, 1.0f, THEME_COLOR_MAIN);
...
static void StatLine(Font font, Rectangle panel, float y, const char *label,
                     const char *value, Color valCol) {
  float fs = 21.0f, padX = 22.0f;
  DrawTextEx(font, label, (Vector2){panel.x + padX, y}, fs, 1.0f,
             Fade(WHITE, 0.85f));
  Vector2 vs = MeasureTextEx(font, value, fs, 1.0f);
  DrawTextEx(font, value, (Vector2){panel.x + panel.width - padX - vs.x, y}, fs,
             1.0f, valCol);
}
...
void DrawTelaGameOver(GameState *game, Font font) {
  ...
  float baseY = 132.0f;
  DrawLineEx((Vector2){120, baseY}, (Vector2){520, baseY}, 2.0f,
             Fade(RED, 0.35f * entry));
  DrawLineEx((Vector2){520, baseY}, (Vector2){560, baseY - 26}, 2.0f,
             Fade(RED, 0.5f * entry));
  DrawLineEx((Vector2){560, baseY - 26}, (Vector2){600, baseY + 30}, 2.0f,
             Fade(RED, 0.5f * entry));
  DrawLineEx((Vector2){600, baseY + 30}, (Vector2){640, baseY}, 2.0f,
             Fade(RED, 0.5f * entry));
  DrawLineEx((Vector2){640, baseY}, (Vector2){1160, baseY}, 2.0f,
             Fade(RED, 0.35f * entry));

  float pulse = 1.0f + sinf(time * 4.0f) * 0.04f;
  DrawTitleText(font, "GAME OVER", SCREEN_WIDTH / 2.0f, 86.0f, 66.0f * pulse,
                Fade((Color){235, 60, 70, 255}, entry));

  // Painel de estatísticas alinhado.
  Rectangle panel = {400, 210, 480, 170};
  DrawPanel(panel, Fade((Color){230, 80, 90, 255}, entry), 0.72f * entry);
  StatLine(font, panel, panel.y + 26, "Pontuacao final",
           TextFormat("%d", game->player.score), GOLD);
  StatLine(font, panel, panel.y + 64, "Nivel final",
           TextFormat("Lvl %d", game->player.level), WHITE);
  StatLine(font, panel, panel.y + 102, "Patogenos eliminados",
           TextFormat("%d", game->totalEnemiesKilled),
           (Color){120, 220, 140, 255});
...
// ============================================================================
// CUTSCENE DO CIENTISTA — reutilizada pela transição pós-Mundo 1 e pela
// vitória. Cientista em destaque (mesma arte do tutorial, grande) + caixa de
// diálogo grande com typewriter. O texto faz wrap e nunca vaza. Avança com
// SPACE/ENTER/Q/clique.
// ============================================================================
int ScientistDialogAdvance(DialogState *dlg, const char *pageText,
                           int pageCount, int voiceScope, float sfxVolume) {
  int totalLen = (int)strlen(pageText);
  float speed = ScientistVoiceCharDelay(totalLen, 0.018f);
  dlg->charTimer += GetFrameTime();
  while (dlg->charTimer >= speed && dlg->charShown < totalLen) {
    dlg->charTimer -= speed;
    dlg->charShown++;
  }
  SyncScientistVoice(voiceScope, dlg->page, dlg->charShown, totalLen,
                     sfxVolume);
...
void DrawScientistDialog(Font font, DialogState *dlg, const char *header,
                         const char *pageText, int pageCount, Color accent,
                         float entry) {
  float t = (float)GetTime();
  int totalLen = (int)strlen(pageText);
  ...
  float dr = 168.0f;
  DrawCircleV(dc, (dr + 26.0f) * (0.98f + 0.02f * sinf(t * 2.0f)),
              Fade(accent, 0.10f * entry)); // halo "transmissão"
  DrawCircleLines((int)dc.x, (int)dc.y, dr + 14.0f,
                  Fade(accent, 0.35f * entry));
  ...
  DrawTextEx(font, who, (Vector2){dc.x - ws.x * 0.5f, dc.y + dr + 22.0f}, 22.0f,
             1.0f, Fade(accent, entry));

  // --- Caixa de diálogo grande (direita) ---
  Rectangle box = {520.0f, 152.0f, 706.0f, 392.0f};
  DrawPanel(box, Fade(accent, entry), 0.85f * entry);
  DrawTextEx(font, header, (Vector2){box.x + 28.0f, box.y + 20.0f}, 22.0f, 1.0f,
             Fade(accent, entry));
  DrawLineEx((Vector2){box.x + 28.0f, box.y + 52.0f},
             (Vector2){box.x + box.width - 28.0f, box.y + 52.0f}, 2.0f,
             Fade(accent, 0.4f * entry));
  ...
  if (n > 1023)
    n = 1023;
  memcpy(shown, pageText, (size_t)n);
  shown[n] = '\0';
  Rectangle textArea = {box.x + 28.0f, box.y + 66.0f, box.width - 56.0f,
                        box.height - 128.0f};
...
int VictoryDialogPages(GameState *game, const char **out) {
  ...
  out[0] =
      "Voce conseguiu, Anticorpo. Contra todas as previsoes, o organismo foi "
...
void DrawTelaVitoria(GameState *game, Font font) {
  ...
  if (game->sceneDialog.active) {
    DrawTitleText(font, "ORGANISMO CURADO!", SCREEN_WIDTH / 2.0f, 54.0f, 30.0f,
                  Fade(THEME_COLOR_TEXT, entry));
    const char *pages[4];
    int pc = VictoryDialogPages(game, pages);
    if (game->sceneDialog.page >= pc)
      game->sceneDialog.page = pc - 1;
    DrawScientistDialog(font, &game->sceneDialog,
                        "TRANSMISSAO // RELATORIO FINAL",
  ...
  for (int r = 0; r < 16; r++) {
    ...
    DrawLineEx(burst,
               (Vector2){burst.x + cosf(a) * len, burst.y + sinf(a) * len},
  ...
  DrawTitleText(font, "ORGANISMO CURADO!", SCREEN_WIDTH / 2.0f, 78.0f,
                60.0f * pulse, Fade(THEME_COLOR_TEXT, entry));

  Rectangle congrats = {240, 150, 800, 34};
  DrawTextWrapped(
      font, "Voce erradicou todas as infeccoes do Distrito Federal!", congrats,
      19.0f, 1.0f, Fade((Color){120, 230, 170, 255}, entry));
  ...
  StatLine(font, panel, panel.y + 64, "Nivel final",
           TextFormat("Lvl %d", game->player.level), WHITE);
  StatLine(font, panel, panel.y + 102, "Total de patogenos",
           TextFormat("%d", game->totalEnemiesKilled),
           (Color){120, 220, 140, 255});
...
void DrawSciFiBox(Rectangle r, Color col) {
  ...
  // Bottom-left corner
  DrawLineEx((Vector2){r.x, r.y + r.height},
             (Vector2){r.x + len, r.y + r.height}, thickness, col);
  DrawLineEx((Vector2){r.x, r.y + r.height},
             (Vector2){r.x, r.y + r.height - len}, thickness, col);
  // Bottom-right corner
  DrawLineEx((Vector2){r.x + r.width, r.y + r.height},
             (Vector2){r.x + r.width - len, r.y + r.height}, thickness, col);
  DrawLineEx((Vector2){r.x + r.width, r.y + r.height},
             (Vector2){r.x + r.width, r.y + r.height - len}, thickness, col);
}
...
void DrawTelaSaveSelect(GameState *game, Font font, Vector2 mouse,
                        Texture2D slotTextures[SAVE_SLOT_COUNT],
                        bool slotTexturesLoaded[SAVE_SLOT_COUNT]) {
  // Fundo escuro translúcido para revelar a run desfocada/limpa no fundo
  DrawRectangle(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT,
                Fade((Color){10, 8, 22, 255}, 0.75f));

  // Título
  const char *titulo = "SAVE PROGRESS";
  Vector2 titleSize = MeasureTextEx(font, titulo, 42.0f, 1.5f);
  DrawTextEx(font, titulo,
             (Vector2){(SCREEN_WIDTH / 2.0f) - (titleSize.x / 2.0f), 40.0f},
             42.0f, 1.5f, SKYBLUE);

  const char *sub = "Choose a manual slot. Autosave is separate and cannot be "
                    "overwritten here.";
  Vector2 subSize = MeasureTextEx(font, sub, 18.0f, 1.0f);
  DrawTextEx(font, sub,
             (Vector2){(SCREEN_WIDTH / 2.0f) - (subSize.x / 2.0f), 90.0f},
             18.0f, 1.0f, GRAY);
  ...
  for (int i = 0; i < MANUAL_SAVE_SLOTS; i++) {
    ...
    if (slotTexturesLoaded[i]) {
      DrawTexturePro(slotTextures[i],
                     (Rectangle){0, 0, (float)slotTextures[i].width,
                                 (float)slotTextures[i].height},
      ...
      DrawTextEx(
          font, "NO PREVIEW",
          (Vector2){imgBounds.x + imgBounds.width / 2.0f - textSz.x / 2.0f,
                    imgBounds.y + imgBounds.height / 2.0f - textSz.y / 2.0f},
          18.0f, 1.0f, DARKGRAY);
    }
    ...
    // Rótulo do Slot
    if (i == 0) {
      DrawTextEx(font, "SLOT 1 (DEFAULT)", (Vector2){cardX + 20, 335}, 22.0f,
                 1.0f, GOLD);
    } else {
      DrawTextEx(font, TextFormat("SLOT %d", i + 1), (Vector2){cardX + 20, 335},
                 22.0f, 1.0f, GOLD);
    }

    if (meta.exists) {
      DrawTextEx(font, TextFormat("Hero: %s", meta.name),
                 (Vector2){cardX + 20, 365}, 18.0f, 1.0f, WHITE);
      DrawTextEx(font, TextFormat("Level: Lvl %d", meta.level),
                 (Vector2){cardX + 20, 390}, 18.0f, 1.0f, WHITE);
      DrawTextEx(font, TextFormat("Wave: %d / 5", meta.wave),
                 (Vector2){cardX + 20, 415}, 18.0f, 1.0f, WHITE);
      DrawTextEx(font, TextFormat("Score: %d", meta.score),
                 (Vector2){cardX + 20, 440}, 18.0f, 1.0f, WHITE);
      DrawTextEx(font, TextFormat("Date: %s", meta.date),
                 (Vector2){cardX + 20, 470}, 14.0f, 1.0f, GRAY);
      ...
      int delFontSize = 16;
      Vector2 delTextSz =
          MeasureTextEx(font, "DELETE SAVE", (float)delFontSize, 1.0f);
      DrawTextEx(
          font, "DELETE SAVE",
          (Vector2){
              deleteBounds.x + deleteBounds.width / 2.0f - delTextSz.x / 2.0f,
              deleteBounds.y + deleteBounds.height / 2.0f - delTextSz.y / 2.0f},
          (float)delFontSize, 1.0f, WHITE);

      // Hover do card como um todo (evitando desenhar se mouse está sobre o
      // botão apagar)
      if (hover && !deleteHover) {
        DrawTextEx(font, "CLICK TO OVERWRITE", (Vector2){cardX + 20, 545},
                   14.0f, 1.0f, RED);
      }
    } else {
      DrawTextEx(font, "EMPTY SLOT", (Vector2){cardX + 20, 365}, 20.0f, 1.0f,
                 DARKGRAY);

      if (hover) {
        DrawTextEx(font, "CLICK TO SAVE GAME", (Vector2){cardX + 20, 545},
                   14.0f, 1.0f, GREEN);
      }
    }
  }
...
void DrawTelaLoadSelect(GameState *game, Font font, Vector2 mouse,
                        Texture2D slotTextures[SAVE_SLOT_COUNT],
                        bool slotTexturesLoaded[SAVE_SLOT_COUNT]) {
  // Fundo escuro translúcido para revelar a run desfocada/limpa no fundo
  DrawRectangle(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT,
  ...
  DrawRectangleRounded(
      autoBounds, 0.05f, 6,
      Fade((Color){20, 18, 32, 255}, autoMeta.exists ? 0.88f : 0.45f));
  DrawRectangleRoundedLines(autoBounds, 0.05f, 6,
                            Fade(autoBorder, autoMeta.exists ? 0.95f : 0.45f));
  DrawTextEx(font, "SAVE AUTOMATICO",
             (Vector2){autoBounds.x + 24, autoBounds.y + 16}, 24.0f, 1.0f,
             GOLD);
  DrawTextEx(
      font,
      "Criado automaticamente durante o gameplay. Nao ocupa slots manuais.",
      (Vector2){autoBounds.x + 24, autoBounds.y + 48}, 16.0f, 1.0f,
      Fade(WHITE, 0.72f));
  if (autoMeta.exists) {
    DrawTextEx(
        font,
        TextFormat("Hero: %s   Lvl %d   Wave %d/5   Score %d", autoMeta.name,
                   autoMeta.level, autoMeta.wave, autoMeta.score),
        (Vector2){autoBounds.x + 24, autoBounds.y + 76}, 16.0f, 1.0f, WHITE);
    DrawTextEx(font, TextFormat("Date: %s", autoMeta.date),
               (Vector2){autoBounds.x + 790, autoBounds.y + 76}, 14.0f, 1.0f,
               GRAY);
  } else {
    DrawTextEx(font, "Nenhum autosave encontrado ainda.",
               (Vector2){autoBounds.x + 24, autoBounds.y + 76}, 16.0f, 1.0f,
               DARKGRAY);
  }
  ...
  for (int i = 0; i < MANUAL_SAVE_SLOTS; i++) {
    ...
    Color cardBg = Fade((Color){20, 18, 32, 255}, 0.85f);
    Color borderCol =
        (hover && meta.exists) ? THEME_COLOR_MAIN : THEME_COLOR_BORDER;
    ...
    // Rótulo do Slot
    if (i == 0) {
      DrawTextEx(font, "SLOT 1 (DEFAULT)", (Vector2){cardX + 20, 410}, 20.0f,
                 1.0f, GOLD);
    } else {
      DrawTextEx(font, TextFormat("SLOT %d", i + 1), (Vector2){cardX + 20, 410},
                 20.0f, 1.0f, GOLD);
    }

    if (meta.exists) {
      DrawTextEx(font, TextFormat("Hero: %s", meta.name),
                 (Vector2){cardX + 20, 438}, 16.0f, 1.0f, WHITE);
      DrawTextEx(font, TextFormat("Level: Lvl %d", meta.level),
                 (Vector2){cardX + 20, 460}, 16.0f, 1.0f, WHITE);
      DrawTextEx(font, TextFormat("Wave: %d / 5", meta.wave),
                 (Vector2){cardX + 20, 482}, 16.0f, 1.0f, WHITE);
      DrawTextEx(font, TextFormat("Score: %d", meta.score),
                 (Vector2){cardX + 20, 504}, 16.0f, 1.0f, WHITE);
      DrawTextEx(font, TextFormat("Date: %s", meta.date),
                 (Vector2){cardX + 20, 526}, 13.0f, 1.0f, GRAY);
      ...
      if (hover && !deleteHover) {
        DrawTextEx(font, "CLICK TO LOAD", (Vector2){cardX + 20, 598}, 13.0f,
                   1.0f, SKYBLUE);
      }
    } else {
      DrawTextEx(font, "EMPTY SLOT", (Vector2){cardX + 20, 438}, 18.0f, 1.0f,
                 DARKGRAY);
    }
  }
...
static void DrawSettingsVolumeSlider(Font font, const char *label,
                                     Rectangle track, float value) {
  value = Clamp(value, 0.0f, 1.0f);
  Vector2 labelSz = MeasureTextEx(font, label, 20.0f, 1.0f);
  DrawTextEx(font, label,
             (Vector2){track.x - labelSz.x - 28.0f, track.y - 6.0f}, 20.0f,
             1.0f, WHITE);

  Rectangle hit = {track.x - 14.0f, track.y - 14.0f, track.width + 28.0f,
                   track.height + 28.0f};
  bool hover = CheckCollisionPointRec(g_virtualMouse, hit);
  DrawRectangleRounded(
      (Rectangle){track.x - 2, track.y - 2, track.width + 4, track.height + 4},
      0.8f, 6, Fade((Color){2, 8, 12, 255}, 0.72f));
  DrawRectangleRounded(track, 0.8f, 6, Fade(BLACK, 0.62f));
  DrawRectangleRounded(
      (Rectangle){track.x, track.y, track.width * value, track.height}, 0.8f, 6,
      THEME_COLOR_MAIN);
  DrawRectangleRoundedLines(
      track, 0.8f, 6,
  ...
  if (knobX < track.x + radius)
    knobX = track.x + radius;
  if (knobX > track.x + track.width - radius)
    knobX = track.x + track.width - radius;
  float knobY = track.y + track.height * 0.5f;
  float t = (float)GetTime();
  Color virusCol =
      hover ? (Color){120, 255, 210, 255} : (Color){80, 220, 190, 255};
  DrawCircleV((Vector2){knobX, knobY}, radius + 2.0f,
              Fade(THEME_COLOR_MAIN, 0.24f + (hover ? 0.18f : 0.0f)));
  DrawMenuVirus((Vector2){knobX, knobY},
                radius * (0.82f + 0.08f * sinf(t * 4.0f)), t * 90.0f, virusCol);
  DrawCircleV((Vector2){knobX, knobY}, 4.0f, Fade(WHITE, 0.88f));

  DrawTextEx(font, TextFormat("%d%%", (int)(value * 100.0f + 0.5f)),
             (Vector2){track.x + track.width + 16.0f, track.y - 5.0f}, 20.0f,
             1.0f, WHITE);
}

void DrawTelaSettings(GameState *game, Font font) {
  DrawThemedBackground(SCREEN_SETTINGS, (float)GetTime(),
                       game->screenAnim / 0.4f);

  const char *title = "CONFIGURACOES";
  Vector2 titleSz = MeasureTextEx(font, title, 42.0f, 1.5f);
  DrawTextEx(font, title, (Vector2){SCREEN_WIDTH / 2.0f - titleSz.x / 2.0f, 60},
             42.0f, 1.5f, SKYBLUE);
  ...
  DrawSettingsVolumeSlider(font, "MUSICA", SettingsMusicVolumeTrack(),
                           game->musicVolume);
  DrawSettingsVolumeSlider(font, "EFEITOS SONOROS", SettingsSfxVolumeTrack(),
                           game->sfxVolume);

  DrawTextEx(font, "Audio salvo automaticamente ao voltar.",
             (Vector2){395, 415}, 14.0f, 1.0f, GRAY);
...
static const char *loadingTips[] = {
    "DICA DE SAUDE: A vacina contra a gripe (Influenza) ajuda a prevenir "
    "complicacoes graves e deve ser tomada anualmente na UBS.",
    "DICA DE SAUDE: Para combater a Dengue, permita a entrada dos agentes da "
    "Vigilancia Ambiental em sua residencia para inspecao.",
    "DICA DE SAUDE: O Distrito Federal possui diversas Unidades Basicas de "
    "Saude (UBS) que oferecem atendimento e vacinacao gratuitos.",
    "DICA DE SAUDE: Lavar as maos frequentemente com agua e sabao e uma das "
    "maneiras mais eficazes de prevenir a transmissao de patogenos.",
    "DICA DE SAUDE: Informe a Vigilancia Ambiental sobre lotes vagos com "
    "acumulo de lixo ou agua parada na sua regiao administrativa.",
    "DICA DE SAUDE: Nao se automedique com antibioticos. O uso incorreto cria "
    "superbacterias resistentes, como a KPC, comuns em hospitais.",
    "DICA DE SAUDE: Use antibioticos apenas com prescricao e complete todo o "
    "tratamento, mesmo que os sintomas melhorem antes.",
    "DICA DE SAUDE: Mascaras e distanciamento reduzem a transmissao de doencas "
    "respiratorias como a gripe (Influenza).",
    "CURIOSIDADE: Bacteriofagos sao virus que infectam bacterias. Vacinas "
    "treinam o sistema imune contra virus de RNA, como dengue e influenza.",
    "CURIOSIDADE: O capsideo e a capa proteica que protege o material genetico "
    "do virus. Neutraliza-lo e essencial para conter a infeccao."};
...
void DrawTelaLoading(GameState *game, Font font) {
  ...
  float entry = game->loadingTimer / 0.35f;
  if (entry > 1.0f)
    entry = 1.0f;
  DrawMenuFXBackground((float)GetTime(), entry);
  // Escurecimento da faixa central/inferior para legibilidade de texto.
  DrawRectangleGradientV(0, 180, SCREEN_WIDTH, 220, Fade(BLACK, 0.0f),
                         Fade(BLACK, 0.45f));
  DrawRectangleGradientV(0, 400, SCREEN_WIDTH, SCREEN_HEIGHT - 400,
                         Fade(BLACK, 0.45f), Fade(BLACK, 0.6f));

  // Partículas decorativas de tecido por cima do fundo (muito sutis).
  for (int i = 0; i < MAX_PARTICLES; i++) {
    if (game->particles[i].active) {
      float s = game->particles[i].size;
      DrawRectangleV((Vector2){game->particles[i].position.x - s,
                               game->particles[i].position.y - s},
                     (Vector2){s * 2.0f, s * 2.0f},
                     Fade(game->particles[i].color,
                          game->particles[i].lifeTime /
                              game->particles[i].maxLifeTime * 0.15f));
    }
  }
  ...
  // brilho
  DrawTextEx(font, titulo, (Vector2){tpos.x, tpos.y}, tfs, 1.5f,
             Fade(tAccent, 0.18f));
  // contorno escuro
  DrawTextEx(font, titulo, (Vector2){tpos.x + 2, tpos.y + 2}, tfs, 1.5f,
             Fade(BLACK, 0.6f));
  ...
  float pct = game->loadingDuration > 0.0f
                  ? (game->loadingTimer / game->loadingDuration)
                  : 1.0f;
  if (pct > 1.0f)
    pct = 1.0f;
  if (pct < 0.0f)
  ...
  DrawTextEx(font, pctText,
             (Vector2){(SCREEN_WIDTH / 2.0f) - (pctSz.x / 2.0f), 316.0f}, 18.0f,
             1.0f, BLACK);
  ...
  DrawTextEx(font, tipTitle,
             (Vector2){(SCREEN_WIDTH / 2.0f) - (tipTitleSz.x / 2.0f), 500.0f},
             16.0f, 1.0f, (Color){0, 220, 120, 255});
  ...
  DrawTextEx(font, line1,
             (Vector2){(SCREEN_WIDTH / 2.0f) - (line1Sz.x / 2.0f), 535.0f},
             15.0f, 1.0f, WHITE);

  if (line2[0] != '\0') {
    Vector2 line2Sz = MeasureTextEx(font, line2, 15.0f, 1.0f);
    DrawTextEx(font, line2,
               (Vector2){(SCREEN_WIDTH / 2.0f) - (line2Sz.x / 2.0f), 565.0f},
               15.0f, 1.0f, WHITE);
  }
}
